...
if st.session_state.current_step == 1:
    ...
    with prompt_container.container():
        ...
        if chatinput:
            info = business_info(query=chatinput)
            if info:
                st.text_input(label="Formation Date", value=info.formation_date)
                st.text_input(label="Business Address", value=info.business_address)
                st.text_input(label="Business Ownership", value=info.business_ownership)
                st.text_input(label="Website URL", value=info.website_url)
                confirm = st.button("Confirm")
                if confirm:
                    st.session_state.responses["response5"] = info
                    update_messages(chatinput, info)
